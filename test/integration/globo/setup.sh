...
vbox_user='vbox'
vbox_vm_ci_string='DevCloud2 original'
...
ShutdownVM() {
    ...
    [[ $debug ]] && PrintLog INFO "VBoxManage controlvm ${vm_uuid} ${shutdowncmd}"
...
RestoreSnapshot() {
    vm_uuid=$1
    [[ $debug ]] && PrintLog INFO "VBoxManage --nologo snapshot ${vm_uuid} restore $snapshot_use_name"
...
TurnOnVM() {
    vm_uuid=$1
    [[ $debug ]] && PrintLog INFO "VBoxManage startvm ${vm_uuid}"
...
WaitForVMState() {
    ...
    while [ $ret_count -le $max_retries ]; do
        ...
        if [[ $vm_status = $wait_for_state ]]; then
            [[ $debug ]] && PrintLog DEBUG "VM state is already in '${wait_for_state}' state"
            return 1
        else
            # arrumar esse echo
            [[ $debug ]] && PrintLog DEBUG "VM state ${vm_uuid} is in '${vm_status}' state, still waiting for '${wait_for_state}' state... sleeping more ${sleep_time}(s) count ${ret_count}/${max_retries}"
            sleep $sleep_time
            ret_count=$[$ret_count+1]
        fi
    done
...
WaitforSSH() {
    ...
    sleep_time=3
    ret_count=1
    [[ $debug ]] && PrintLog DEBUG "Trying to connect to ${host_to_connect}..."
    while [ $ret_count -le $max_retries ]; do
        if echo "QUIT" | nc -w 2 ${host_to_connect} 22 >/dev/null 2>&1; then
            [[ $debug ]] && PrintLog INFO "SSH service is ok on host ${host_to_connect}"
            return 1
        else
            [[ $debug ]] && PrintLog DEBUG "SSH is not ready yet...sleeping more ${sleep_time}(s) count ${ret_count}/${max_retries}"
            sleep $sleep_time
            ret_count=$[$ret_count+1]
        fi
    done
    # return 0
    PrintLog ERROR "Ssh service is not running in vm ${vm_uuid}..."
    exit 1
}
...
[[ $debug ]] && PrintLog INFO "Removing log file '${maven_log}'"
rm -f ${maven_log}
...
# inserir alguns check para verificar a disponibilidade do ambiente
